...
const EntryRegistration: React.FC<EntryRegistrationProps> = ({ navigate, onSuccess, userRole }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f6f8f7]">
      ...
      <main className="flex-1 p-4 flex flex-col gap-6 pb-10 overflow-y-auto no-scrollbar md:p-8">
        ...
        <div className="md:grid md:grid-cols-2 md:gap-8 md:items-start">
          <div className="space-y-6">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full h-14 bg-white border border-gray-100 rounded-2xl pl-12 pr-4 text-base focus:border-[#10c65c] outline-none transition-colors"
            ...
            <section>
              ...
              <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 lg:grid-cols-4">
                {materials.map((m) => (
                  <button key={m.id} onClick={() => { setSelectedMaterial(m); setSelectedSubclass(null); }} className={`relative flex flex-col items-center justify-center h-24 rounded-2xl border-2 transition-all ${selectedMaterial?.id === m.id ? 'border-[#10c65c] bg-[#10c65c]/5 scale-105' : 'border-transparent bg-white shadow-sm hover:shadow-md hover:scale-105'}`}>
          ...
          <div className="space-y-6 md:bg-white md:p-6 md:rounded-3xl md:border md:border-gray-100 md:shadow-sm md:sticky md:top-4">
            <section className="flex flex-col gap-4">
              <label className="text-xs font-bold text-gray-500 ml-1">Peso (kg)</label>
              <div className="h-20 bg-white md:bg-gray-50 rounded-3xl border-2 border-[#10c65c]/20 flex items-center justify-center px-8 text-5xl font-black text-gray-800 transition-colors">
                {weight}<span className="text-xl text-gray-300 ml-2 mt-2">kg</span>
              </div>
              <div className="grid grid-cols-3 gap-3">
                {['1', '2', '3', '4', '5', '6', '7', '8', '9', '.', '0', 'delete'].map(k => (
                  <button key={k} onClick={() => handleKeypad(k)} className="h-14 bg-white md:bg-gray-50 rounded-xl flex items-center justify-center text-xl font-bold shadow-sm active:bg-gray-100 md:hover:bg-gray-200 transition-colors border border-transparent md:border-gray-200">
            ...
            <div className="flex flex-col gap-3 pt-4 border-t border-gray-50">
              <button onClick={handleSave} disabled={loading} className="h-16 w-full bg-[#10c65c] text-white rounded-2xl flex items-center justify-center gap-2 font-bold shadow-lg shadow-[#10c65c]/20 active:scale-95 transition-all disabled:opacity-50 hover:bg-[#0da54b] hover:shadow-xl">
                <span className="material-symbols-outlined">check</span>
                {loading ? 'SALVANDO...' : (userRole === 'associate' ? 'CONFIRMAR ENTREGA' : 'SALVAR CADASTRO')}
              </button>

              <button onClick={() => navigate('ENTRIES_HISTORY')} className="h-14 w-full bg-white text-gray-700 border-2 border-gray-100 rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-all hover:bg-gray-50 hover:border-gray-300">
